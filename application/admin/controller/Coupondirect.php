...
class Coupondirect extends MainController
{
	...
	public function __construct() {
        parent::__construct();
		$this->DBTextConnecter = DBTextConnecter::withTableName('coupon_direct');
	...
	public function delete() {
        ...
        try{
            ...
            Db::table($this->associated_tableName)->where('coupon_id',$id)->delete();
        } catch (\Exception $e){
            $this->dumpException($e);
        }
    ...
    public function createAct() {
        ...
        try{
            ...
            $count = $this->getNumber();
            $newData['number'] = config('subDeparment') . 'T' . date('Ymd') . $count;
            ...
            if($actData['img']){
                ...
                Db::table($this->tableName)->where('id', $insertGetId)->update($updateData);
            }
        ...
        Db::table($this->associated_tableName)->insertAll($actProd);
        $echoData = [
            'status'    => 200,
            'actData'   => $actData,
            'actProd'   => $actProd
    ...
    public function getActProd(){
        ...
        $actProd = Db::table($this->associated_tableName)->alias('ap')
                    ->join('productinfo pi','ap.prod_id = pi.id')
                    ->where('ap.coupon_id',$actId)->select();
	...
	public function getCount(){
		try{
			$act_count = Db::table($this->tableName)->where("online = '1' and ( (start <= '".time()."' and end >= '".time()."' ) or  (start <= '".time()."' and end = '-28800' ) )")->count();
			$outputData = [
				'status' => true,
				'message' => $act_count
			];
    ...
    public function getNumber(){
        $count = Db::table($this->tableName)->where('number like "'.config('subDeparment').'T'.date('Ymd').'%"')->order('id desc')->find();
